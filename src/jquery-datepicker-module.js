...
/**
 * 日期插件
 * 模块化方式定义，jQuery插件形式调用
 * 结合textposition插件进行输入控制（无需再引入textposition插件）
 * @author ifuyun
 * @version 1.0
 */
    ...
    (function ($) {
        ...
        $.Datepicker.initGlobalEvent = function () {
            $(document).mousedown(function (e) {
                ...
                if (that) {
                    ...
                    if ($picker.find('.monthDiv').is(':visible') && (!$target.is('.monthDiv') && $target.parents('.monthDiv').length < 1 && !$target.is('.monthInput'))) {
                        that.hideMonthTable();
                    }
                    if ($picker.find('.yearDiv').is(':visible') && (!$target.is('.yearDiv') && $target.parents('.yearDiv').length < 1 && !$target.is('.yearInput'))) {
                        that.hideYearTable();
                    }
                }
            });
        ...
        $.Datepicker.prototype = {
            ...
            initInputEvent: function () {//输入事件
                var that = this, $picker = this.$picker;
                this.$elm.on(this.opts.showOn, function (e) {
                    ...
                    if (keyCode === 109 || keyCode === 173) {//-
                        if (posObj.start !== 4 && posObj.start !== 7) {//非正常位置禁止输入-号
                            return false;
                        }
                        return true;
                    }
            ...
            initPickerEvent: function () {//日期控件事件，定义控件行为
                ...
                $picker.on('mouseover mouseout', '.dateDiv td.pickDay', function (e) {
                    ...
                    //待增加回车事件
                    var actYear = parseInt($picker.find('.yearInput').val(), 10), actMonth = parseInt($(this).attr('monthData'), 10), actDay = parseInt($picker.find('.active').html(), 10), monthDays;
                    ...
                    //待增加回车事件
                    var actYear = parseInt($(this).html(), 10), actMonth = parseInt($picker.find('.monthInput').val(), 10), actDay = parseInt($picker.find('.active').html(), 10), monthDays, minDate = that.getDateObj(that.opts.minDate), maxDate = that.getDateObj(that.opts.maxDate);
                    monthDays = that.getMonthDays(actYear, actMonth);
                    actDay = actDay > monthDays ? monthDays : actDay;
                    ...
                    var actYear = parseInt($picker.find('.yearInput').val(), 10) - 1, actMonth = parseInt($picker.find('.monthInput').val(), 10), actDay = parseInt($picker.find('.active').html(), 10), monthDays, minDate = that.getDateObj(that.opts.minDate);
                    ...
                    var actYear = parseInt($picker.find('.yearInput').val(), 10), actMonth = parseInt($picker.find('.monthInput').val(), 10), actDay = parseInt($picker.find('.active').html(), 10), monthDays, minDate = that.getDateObj(that.opts.minDate);
                    if (actMonth === 1) {
                        actMonth = 12;
                        actYear -= 1;
                    } else {
                    ...
                    var actYear = parseInt($picker.find('.yearInput').val(), 10), actMonth = parseInt($picker.find('.monthInput').val(), 10), actDay = parseInt($picker.find('.active').html(), 10), monthDays, maxDate = that.getDateObj(that.opts.maxDate);
                    if (actMonth === 12) {
                        actMonth = 1;
                        actYear += 1;
                    } else {
                    ...
                    var actYear = parseInt($picker.find('.yearInput').val(), 10) + 1, actMonth = parseInt($picker.find('.monthInput').val(), 10), actDay = parseInt($picker.find('.active').html(), 10), monthDays, maxDate = that.getDateObj(that.opts.maxDate);
            ...
            calcPos: function () {//位置计算
                var $elm = this.$elm, elmOffset = $elm.offset(), posObj = {
                    l: elmOffset.left,
                    t: elmOffset.top,
                    w: $elm.outerWidth(),
                    h: $elm.outerHeight(),
                    sl: $elm.scrollLeft(),
                    st: $elm.scrollTop(),
                    ol: $elm.get(0).offsetLeft,
                    ot: $elm.get(0).offsetTop
                };
            ...
            showByDate: function (dateObj) {//根据指定日期显示控件
                var $picker = this.$picker, curDateObj = this.getCurDate();
            ...
            refreshYearCtrl: function (actYear) {//刷新前、后十年控制按钮
                var $picker = this.$picker, minDate = this.getDateObj(this.opts.minDate), maxDate = this.getDateObj(this.opts.maxDate);
            ...
            getMonthTable: function (actYear) {//生成月份表格
                //待加日期范围限制
                var i = 0, tblHtml = '', monthClass, minDate = this.getDateObj(this.opts.minDate), maxDate = this.getDateObj(this.opts.maxDate);
                for ( i = 0; i < 6; i += 1) {
                    //重置
                    monthClass = [];
            ...
            getYearTable: function (actYear) {//生成年份表格
                //待加日期范围限制
                var i = 0, tblHtml = '', yearClass, minDate = this.getDateObj(this.opts.minDate), maxDate = this.getDateObj(this.opts.maxDate);
                for ( i = 0; i < 5; i += 1) {
                    //重置
                    yearClass = [];
            ...
            getDateTable: function (curDateObj, actDateObj, firstWeek) {//生成日期表格
                ...
                isCurYearMonth = $picker.find('.yearInput').val() === curDateObj.curYear.toString() && $picker.find('.monthInput').val() === curDateObj.curMonth.toString();
            ...
            getActiveDate: function () {//返回当前选择的日期
                var dateStr = this.$elm.val(), dateArr = [];
            ...
            getDateObj: function (mixDate) {//解析日期对象，返回年月日，支持Date、字符串、数值类型
                var dateType = typeof mixDate, rawDateType = Object.prototype.toString.call(mixDate), dateObj = null, tmpArr = [];
            ...
            processError: function () {
                ...
                var opts = this.opts, $elm = this.$elm, isValid = false, actDateStr = $elm.val(), actDateObj, minDate = this.getDateObj(opts.minDate), maxDate = this.getDateObj(opts.maxDate), returnVal = true;
